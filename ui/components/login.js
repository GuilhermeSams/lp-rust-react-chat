...
export default function Login({ show, setAuth }) {
  ...
  const FormCreateUsername = ({ setAuth }) => {
    ...
    return (
      <form action="" className="mt-4 space-y-2" onSubmit={onCreateUsername}>
        <div>
          <label className="text-sm font-light">Seu nome</label>
          <input
            required
            type="text"
            name="username"
        ...
        <div>
          <label className="text-sm font-light">Número de telefone</label>
          <input
            required
            type="text"
            name="phone"
  ...
  return (
    <div
      className={`${
        show ? "" : "hidden"
      } bg-gradient-to-t from-slate-800 to-indigo-600`}
    >
      <div className="flex items-center justify-center min-h-screen">
        <div className="px-8 py-6 mt-4 text-left bg-white  max-w-[400px] w-full rounded-xl shadow-lg">
          <h3 className="text-xl text-slate-800 font-semibold">
            {isShowSigIn ? "Logar com telefone" : "Crie sua conta"}
          </h3>
          {isShowSigIn ? (
